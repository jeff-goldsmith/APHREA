...
This page builds on content in [dashboards](shortcourse_dashboards.html).
...
As in [dashboards](shortcourse_dashboards.html), we're going to focus on the [Airbnb](data.html) data. Our goal is to make a dashboard similar to the one produced there, but with interactive elements in place of hard-coded data manipulation choices. 
...
In [dashboards](shortcourse_dashboards.html), we made plots showing rental locations, number of rentals in each neighborhood, and the price range of rentals in some neighborhoods. We're going to make similar plots now, but we want these to update based on user inputs. 
