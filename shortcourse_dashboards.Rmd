...
We're going to focus on the [Airbnb](data.html) data for this page. The code below extracts what we need right now; specifically, we select only a few of the variables and filter to include a subset of the data. In part, this makes sure that the resulting dataset and plots are computationally feasible -- for large datasets, you may need to downsample.
